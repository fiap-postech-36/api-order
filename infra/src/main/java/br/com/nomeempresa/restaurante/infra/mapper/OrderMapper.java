...
public interface OrderMapper {
    ...
    default List<ProductEntity> mapProdutosToProdutoItems(List<Product> produtos) {
        return produtos.stream().map(this::mapProdutoToProdutoItem).collect(Collectors.toList());
    }

    default ProductEntity mapProdutoToProdutoItem(Product product) {
        return new ProductEntity(product.getId(), product.getName(), product.getDescription(), product.getUrlImage(), product.getPrice(), product.getCategory());
    }
